...
export function CameraFeed() {
  ...
  // Initialize speech synthesis with new iOS flag
  const { speak, isSafari, isIOS, initializeAudioContext } = useSpeech();
  const audioContextInitializedRef = useRef(false); // Track if audio context is initialized
  ...
  const handleUserInteraction = useCallback(async () => {
    if (!hasUserInteracted) {
      setHasUserInteracted(true);
      // Initialize audio context for iOS Safari only if not already initialized
      if ((isSafari || isIOS) && !audioContextInitializedRef.current) {
        await initializeAudioContext();
    ...
    if (!hasVisitedBefore) {
      ...
      if (isSafari || isIOS) {
        const playWelcomeMessage = async () => {
          await handleUserInteraction(); // Ensure audio context is initialized
          speak("Welcome to Blind-Spot, say describe the scene, or tap the screen to get Started.");
          // Remove the event listeners after playing the message
          document.removeEventListener('click', playWelcomeMessage);
          document.removeEventListener('touchstart', playWelcomeMessage);
  ...
  }, [startCamera, describeScene, speak, isSafari, isIOS, handleUserInteraction]);
